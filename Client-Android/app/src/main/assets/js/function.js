function showMessage(message, level, noTimeout = false) {
    if (!message) return;
    // 获取或创建消息容器
    ...
    // 点击消息时移除
    messageDiv.addEventListener('click', () => messageDiv.parentNode.removeChild(messageDiv));

    // 延时移除
    if (!noTimeout) setTimeout(() => messageDiv.parentNode.removeChild(messageDiv), 3000);
