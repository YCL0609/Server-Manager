export default {
    init: {
        argErr: '出现参数冲突，程序退出!',
        statErr: '初始化错误: 无法获取配置文件属性信息! - Code',
        readErr: '初始化错误: 无法读取配置文件!',
        parseErr: '初始化错误: 配置文件不合规!',
        inrunning: '已有实例在运行中，程序退出.',
        initSuccess: '主进程启动成功 - Pid:',
        lockFileErr: '初始化错误: 无法获取锁文件!',
        childProcExit: '子进程退出',
        childProcessErr: '初始化错误: 一个或多个子进程启动失败!',
    },
    public: {
        initErr: '初始化错误! - Code:',
        timeout: '命令操作超时, 已自动终止命令!',
        isDirErr: '数据文件路径指向一个目录而非文件!',
        cleanTip: '执行退出清理操作...',
        removeErr: '无法删除旧数据文件!',
        symlinkErr: '无法创建符号链接!',
        writeFileErr: '无法写入数据文件!',
        intervalWarn: '初始化警告: 监控间隔小于1000ms可能存在性能问题!',
        errorCount: '连续失败超过 10 次，自动终止!',
        startTimerErr: '无法启动定时器!',
    },
    dirCheck: {
        errNewDir: 'dirCheck(): 无法创建目录!',
        fileExist: 'dirCheck(): 路径已存在且不是目录, 函数提前返回! - 路径:',
        illegalPath: 'dirCheck(): 检测到非法路径段, 函数提前返回!',
    },
    helpTipShow: {
        option: '选项',
        usage: '用法',
        detal: '一个轻量级的服务器系统监控与服务管理工具',
        cfgDetal: '指定配置文件路径 (默认: /etc/server_manager.cfg)',
        helpDetal: '显示帮助信息并退出',
        permDetal: '由于服务控制模块涉及 systemctl 操作, 请确 /etc/sudoers 中存在以下行',
        permission: '权限配置',
        sysmonDetal: '以系统监控模式启动 (通常由主进程自动调用)',
        srvctlDetal: '以服务控制模式启动 (通常由主进程自动调用)',
    },
    SysMonitor: {
        initSuccess: '系统监控模块启动成功 - Pid:',
        readInfoErr: 'SysMonitor(): 无法读取系统状态信息!',
    },
    SvrMonitor: {
        initSuccess: '服务监控模块启动成功 - Pid:',
        readInfoErr: 'ServiceMonitor(): 无法读取系统服务状态信息!',
    },
    SvrControl: {
        initSuccess: '服务控制模块启动成功 - Pid:',
        pidGetErr: 'ServiceControl(): 获取服务监控进程pid错误, 监控数据可能出现延迟!',
        ctrlFileErr: 'ServiceControl(): 服务控制文件初始化失败!',
        switchErr: 'ServiceControl(): 尝试切换服务状态失败!',
        switchSuccess: 'ServiceControl(): 服务状态切换成功!',
    },
}